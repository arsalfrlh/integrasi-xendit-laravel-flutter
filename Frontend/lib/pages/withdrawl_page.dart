...
class _WithdrawlPageState extends State<WithdrawlPage> {
  ...
  final nomorBankController = TextEditingController();
  final jumlahController = TextEditingController();

  void _tarik(BuildContext context)async{
    if(nameController.text.isNotEmpty && bankController.text.isNotEmpty && nomorBankController.text.isNotEmpty && jumlahController.text.isNotEmpty){
      showDialog(
        context: context, 
        barrierDismissible: false,
        builder: (context) => const Center(child: CircularProgressIndicator(),));

      final response = await apiService.createWithdrawl(int.parse(jumlahController.text), bankController.text, nameController.text, nomorBankController.text);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("Withdrawl Page"),
      ),
      body: SafeArea(
        child: SizedBox(
              ...
              child: Column(
                children: [
                  ...
                  SignUpForm(
                    name: nameController,
                    bank: bankController,
                    nomor: nomorBankController,
                    jumlah: jumlahController,
                    tarik: () => _tarik(context),
                  ),
                ],
              ),
            ),
...
class SignUpForm extends StatefulWidget {
  SignUpForm({required this.name, required this.bank, required this.nomor, required this.jumlah, required this.tarik});
  final VoidCallback tarik;
  final TextEditingController name, bank, nomor, jumlah;
      ...
      child: Column(
        children: [
          ...
          Padding(
            ...
            child: TextFormField(
              controller: widget.nomor,
              textInputAction: TextInputAction.next,
              decoration: InputDecoration(
                  hintText: "Enter your Nomor Bank Account",
                  labelText: "Nomor",
                  floatingLabelBehavior: FloatingLabelBehavior.always,
                  hintStyle: const TextStyle(color: Color(0xFF757575)),
                  contentPadding: const EdgeInsets.symmetric(
          ...
          TextFormField(
            ...
            decoration: InputDecoration(
                hintText: "Enter your value withdrawl",
                labelText: "Value",
                floatingLabelBehavior: FloatingLabelBehavior.always,
                hintStyle: const TextStyle(color: Color(0xFF757575)),
                contentPadding: const EdgeInsets.symmetric(
