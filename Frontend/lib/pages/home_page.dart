...
import 'package:xendit/pages/bank_page.dart';
import 'package:xendit/pages/qr_page.dart';
import 'package:xendit/pages/store_page.dart';
import 'package:xendit/pages/withdrawl_page.dart';
import 'package:xendit/services/api_service.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  void _payment(BuildContext context)async{
    if(nameController.text.isNotEmpty && jumlahController.text.isNotEmpty && methodController.text.isNotEmpty){
      showDialog(
        context: context, 
        barrierDismissible: false,
        builder: (context) => const Center(child: CircularProgressIndicator(),));

      final response = await apiService.createPayment(nameController.text, int.parse(jumlahController.text), bankController.text, storeController.text, methodController.text);
      ...
      if(response['success'] == true){
        ...
        if(response['method'] == 'bank'){
          final payment = Payment(
            ...
            expire: DateTime.parse(data['expiration_date']),
            total: data['expected_amount'],
            paymentCode: data['account_number'],
            bank: data['bank_code'],
          );
          ...
          AwesomeDialog(
            ...
            btnOkOnPress: (){
              Navigator.push(context, MaterialPageRoute(builder: (context) => BankPage(payment: payment)));
            },
          ).show();

        }else if(response['method'] == 'store'){
          final payment = Payment(
            id: data['id'],
            referenceID: data['external_id'],
            expire: DateTime.parse(data['expiration_date']),
          ...
          AwesomeDialog(
            ...
            desc: response['message'],
            btnOkOnPress: (){
              Navigator.push(context, MaterialPageRoute(builder: (context) => StorePage(payment: payment)));
            },
          ).show();
          ...
          final payment = Payment(
            ...
            expire: DateTime.parse(data['payment']['expires_at']),
          ...
          AwesomeDialog(
            ...
            desc: response['message'],
            btnOkOnPress: (){
              Navigator.push(context, MaterialPageRoute(builder: (context) => QrPage(payment: payment)));
            },
          ).show();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("Payment Xendit"),
        actions: [
          IconButton(onPressed: (){
            Navigator.push(context, MaterialPageRoute(builder: (context) => WithdrawlPage()));
...
class SignUpForm extends StatefulWidget {
  const SignUpForm({required this.name, required this.jumlah, required this.bank, required this.store, required this.method, required this.payment});
  final TextEditingController name, jumlah, bank, store, method;
...
class _SignUpFormState extends State<SignUpForm> {
  final Map<String, dynamic> methodOptions = {
    "bank": "Bank",
    "store": "Store",
    "qr": "QR Code",
  };

  final Map<String, dynamic> bankOptions = {
  ...
  final Map<String, dynamic> storeOptions = {
    "ALFAMART": "Alfamaret",
    "INDOMARET": "Indomaret",
  };
      ...
      child: Column(
        children: [
          ...
          DropdownButtonFormField<String>(
            value: widget.method.text.isNotEmpty
            ? widget.method.text
            : null,
            items: methodOptions.entries.map((entry){
